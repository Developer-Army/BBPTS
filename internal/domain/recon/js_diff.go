...
var fallbackEndpointPatterns = []*regexp.Regexp{
	...
	regexp.MustCompile(`['"](/admin[a-zA-Z0-9_/\-{}:.]*?)['"]`),
...
func extractBinding(binding *ast.Binding, routes *[]SemanticRoute) {
	if str, ok := binding.Initializer.(*ast.StringLiteral); ok {
		val := stringLiteralValue(str)
		if strings.HasPrefix(val, "/") && (strings.Contains(val, "api") || strings.Contains(val, "/v") || strings.Contains(val, "graphql") || strings.Contains(val, "admin")) {
			var varName string
			if ident, ok := binding.Target.(*ast.Identifier); ok {
				varName = ident.Name.String()
			}
			*routes = append(*routes, SemanticRoute{
				Path:       val,
