...
func (t *GithubOrgDiscoveryTool) Run(ctx context.Context, scanCtx *recon.ScanContext, targets []string, threads int) ([]recon.Event, error) {
...
func (t *GithubOrgDiscoveryTool) discover(ctx context.Context, apiKey, orgOrDomain string) ([]recon.Event, error) {
	events := []recon.Event{}
	...
	if orgInfo != nil {
		...
		if blog := orgInfo["blog"]; blog != "" {
			events = append(events, recon.NewEvent(blog, t.Name(), "discovery", map[string]string{
		...
		if email := orgInfo["email"]; email != "" {
			events = append(events, recon.NewEvent(email, t.Name(), "discovery", map[string]string{
...
func (t *GithubOrgDiscoveryTool) listRepos(ctx context.Context, apiKey, org string) []recon.Event {
	events := []recon.Event{}
	client := NewSafeHTTPClient(12 * time.Second)
	page := 1
	...
	for {
		...
		for _, r := range list {
			events = append(events, recon.NewEvent(r.FullName, t.Name(), "discovery", map[string]string{
				"repo":        r.Name,
			...
			if r.Homepage != "" {
				events = append(events, recon.NewEvent(r.Homepage, t.Name(), "discovery", map[string]string{
...
func (t *GithubOrgDiscoveryTool) listMembers(ctx context.Context, apiKey, org string) []recon.Event {
	events := []recon.Event{}
	client := NewSafeHTTPClient(12 * time.Second)
	page := 1
	...
	for {
		...
		for _, m := range list {
			...
			events = append(events, recon.NewEvent(m.Login, t.Name(), "github_account", map[string]string{
...
func (t *GithubOrgDiscoveryTool) scanWebhooks(ctx context.Context, apiKey, org string) []recon.Event {
	events := []recon.Event{}
	...
	for _, h := range hooks {
		events = append(events, recon.NewEvent(h.Config.URL, t.Name(), "discovery", map[string]string{
