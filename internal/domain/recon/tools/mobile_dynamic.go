...
func (t *MobileDynamicTool) Run(ctx context.Context, scanCtx *recon.ScanContext, targets []string, threads int) ([]recon.Event, error) {
	if len(targets) == 0 {
		return nil, nil
	}
...
func (t *MobileDynamicTool) analyzeTarget(ctx context.Context, target string, frida, objection bool) []recon.Event {
	events := []recon.Event{}
...
func (t *MobileDynamicTool) runFrida(ctx context.Context, target string) []recon.Event {
	events := []recon.Event{}
...
func (t *MobileDynamicTool) parseFridaOutput(output, target, scriptName string) []recon.Event {
	events := []recon.Event{}
...
func (t *MobileDynamicTool) runObjection(ctx context.Context, target string) []recon.Event {
	events := []recon.Event{}
...
func (t *MobileDynamicTool) passiveScan(targets []string) []recon.Event {
	events := []recon.Event{}
	for _, target := range targets {
		target = strings.TrimSpace(target)
		if target == "" {
...
func (t *MobileDynamicTool) passiveHints(target string) []recon.Event {
	events := []recon.Event{}
