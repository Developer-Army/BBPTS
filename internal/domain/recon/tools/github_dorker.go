...
func (t *GithubDorkerTool) Run(ctx context.Context, scanCtx *recon.ScanContext, targets []string, threads int) ([]recon.Event, error) {
			...
			for _, dork := range queries {
				...
				for _, item := range items {
					...
					for _, sm := range secretMatches {
						...
						events = append(events, recon.NewEventWithSeverity(
	...
	for _, member := range members {
		...
		for _, repo := range memberRepos {
			...
			for _, match := range subMatches {
				...
				events = append(events, recon.NewEvent(match, t.Name(), "subdomain", props))
			}
		}
	}
