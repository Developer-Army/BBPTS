...
func GetLLMConfig(ctx context.Context) (provider, model, apiURL, apiKey string) {
	provider = "gemini"
	model = "gemini-2.5-flash"
	apiKey = recon.GetAPIKey(ctx, "gemini")
	if apiKey == "" {
		apiKey = os.Getenv("GEMINI_API_KEY")
	}
	if apiKey == "" {
		if key := recon.GetAPIKey(ctx, "anthropic"); key != "" || os.Getenv("ANTHROPIC_API_KEY") != "" {
			provider = "anthropic"
			model = "claude-sonnet-4-6"
			apiKey = key
			if apiKey == "" {
	...
	if apiKey == "" {
		if key := recon.GetAPIKey(ctx, "openai"); key != "" || os.Getenv("OPENAI_API_KEY") != "" {
			provider = "openai"
			model = "gpt-4o"
			apiKey = key
			if apiKey == "" {
