...
func (j *JSAnalyzer) Run(ctx context.Context, scanCtx *recon.ScanContext, targets []string, threads int) ([]recon.Event, error) {
	if len(targets) == 0 {
		return nil, nil
	}
...
func (j *JSAnalyzer) analyzeJS(ctx context.Context, client *network.StealthClient, url string) []recon.Event {
	...
	for _, match := range gqlOps {
		events = append(events, recon.NewEvent(match[2], j.Name(), "graphql_operation", map[string]string{
			"source":  url,
			"op_type": match[1],
		}))
	}
	...
	if store != nil {
		...
		if err == nil && prevEv != nil && prevEv.Hash != currentHash {
			...
			if diffText != "" {
				events = append(events, recon.NewEventWithSeverity(url, j.Name(), "vulnerability", map[string]string{
	...
	for _, f := range findings {
		...
		case "framework", "lazy_route", "sourcemap":
			events = append(events, recon.NewEvent(url, j.Name(), "discovery", map[string]string{
				"source": url,
...
Output ONLY valid JSON, no markdown.`, sourceURL, i+1, len(chunks), chunk)
		...
		for _, f := range findings {
			...
			switch f.Type {
			case "endpoint":
				events = append(events, recon.NewEvent(f.Value, j.Name(), "api_endpoint", map[string]string{
					"source":      sourceURL,
			...
			case "hardcoded_value":
				...
				events = append(events, recon.NewEventWithSeverity(sourceURL, j.Name(), "vulnerability", map[string]string{
			...
			case "auth_logic":
				events = append(events, recon.NewEvent(sourceURL, j.Name(), "discovery", map[string]string{
					"source":      sourceURL,
					"type":        "auth_logic",
					"detail":      f.Value,
					"ai_analysis": f.Description,
			...
			case "parameter":
				events = append(events, recon.NewEvent(f.Value, j.Name(), "discovery", map[string]string{
					"source":      sourceURL,
					"type":        "js_parameter",
					"ai_analysis": f.Description,
				}))
