...
func (t *Wafw00fTool) Run(ctx context.Context, scanCtx *recon.ScanContext, targets []string, threads int) ([]recon.Event, error) {
	if len(targets) == 0 {
		return nil, nil
	}
			...
			if strings.Contains(strings.ToLower(line), "identified") || strings.Contains(strings.ToLower(line), "detected") {
				...
				if idx := strings.LastIndex(line, ":"); idx != -1 {
					...
					if wafName != "" && wafName != "None" {
						...
						events = append(events, recon.NewEvent(target, t.Name(), "waf-detection", props))
					}
				}
			}
		}
