...
func (p *SwaggerParser) Parse(specContent []byte, parsedSpecURL *url.URL) ([]recon.Event, []string, error) {
	...
	for name, scheme := range securitySchemes {
		...
		events = append(events, recon.NewEvent(parsedSpecURL.String(), "swagger_parser", "discovery", props))
	}
	...
	for pathStr, pathVal := range pathsObj {
		...
		for method, methodVal := range pathMap {
			...
			events = append(events, recon.NewEvent(fullURL, "swagger_parser", "api_endpoint", props))
		}
	}
