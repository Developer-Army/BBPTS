...
func (t *CloudBucketsTool) Run(ctx context.Context, scanCtx *recon.ScanContext, targets []string, threads int) ([]recon.Event, error) {
	if len(targets) == 0 {
		return nil, nil
	}
...
func (t *CloudBucketsTool) probeBucket(ctx context.Context, client *http.Client, url, provider, bucket string, events *[]recon.Event, mu *sync.Mutex) {
