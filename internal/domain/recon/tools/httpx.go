...
func (t *HTTPXTool) Run(ctx context.Context, scanCtx *recon.ScanContext, targets []string, threads int) ([]recon.Event, error) {
	if len(targets) == 0 {
		return nil, nil
	}
	...
	for _, line := range lines {
		...
		events = append(events, recon.NewEvent(out.URL, t.Name(), "service", props))
