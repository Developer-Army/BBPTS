...
func (t *OAuthTesterTool) Run(ctx context.Context, scanCtx *recon.ScanContext, targets []string, threads int) ([]recon.Event, error) {
	if len(targets) == 0 {
		return nil, nil
	}
	...
	return pool.Process(ctx, targets, func(ctx context.Context, target string) ([]recon.Event, error) {
		...
		for _, candidate := range dedupeStrings(candidates) {
			...
			if q.Get("state") != "" {
				...
				if testErr == nil && ok {
					events = append(events, recon.NewEventWithSeverity(candidate, t.Name(), "vulnerability", map[string]string{
...
var _ recon.Tool = (*OAuthTesterTool)(nil)
