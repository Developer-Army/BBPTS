...
func (t *SQLMapTool) Run(ctx context.Context, targets []string, threads int) ([]Event, error) {
	if len(targets) == 0 {
		return nil, nil
	}
	if !ExploitSQLIFromCtx(ctx) {
	...
	return pool.Process(ctx, sqlTargets, func(ctx context.Context, target string) ([]Event, error) {
		return t.testTarget(ctx, target), nil
	})
}

// testTarget runs SQLMap against a single target URL.
func (t *SQLMapTool) testTarget(ctx context.Context, target string) []Event {
	var events []Event
