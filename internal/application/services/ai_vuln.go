...
func (a *AIVulnTool) Run(ctx context.Context, targets []string, threads int) ([]Event, error) {
	if len(targets) == 0 {
		return nil, nil
	}
	...
	return pool.Process(ctx, httpTargets, func(ctx context.Context, target string) ([]Event, error) {
...
func (a *AIVulnTool) analyzeTarget(ctx context.Context, client *network.StealthClient, baseURL string) []Event {
	var events []Event

	// Phase 1: Detect AI endpoints
	aiEndpoints := a.detectAIEndpoints(ctx, client, baseURL)
	for _, ep := range aiEndpoints {
		events = append(events, NewEventWithSeverity(ep, a.Name(), "discovery", map[string]string{
			"type":        "ai_endpoint",
	...
	if mainPageAI {
		events = append(events, NewEvent(baseURL, a.Name(), "discovery", map[string]string{
...
func (a *AIVulnTool) testPromptInjection(ctx context.Context, client *network.StealthClient, endpoint string) []Event {
	var events []Event
	...
	for _, pi := range promptInjectionPayloads {
		...
		for _, respBody := range responses {
			...
			if leakScore >= 2 || (leakScore >= 1 && lengthRatio > 3.0) {
				...
				events = append(events, NewEventWithSeverity(endpoint, a.Name(), "vulnerability", map[string]string{
