...
func (t *EmailEnumTool) Run(ctx context.Context, targets []string, threads int) ([]Event, error) {
	hunterKey := GetAPIKey(ctx, "hunter")
	if hunterKey == "" {
		slog.Debug("Hunter.io API key not configured, skipping email_enum")
		return nil, nil
	}

	githubKey := GetAPIKey(ctx, "github")
	...
	for _, target := range targets {
		...
		go func(dom string) {
			...
			events := t.processDomain(ctx, dom, hunterKey, githubKey, &seen, &mu)
			mu.Lock()
...
func (t *EmailEnumTool) processDomain(
	...
	hunterResp, err := t.queryHunter(ctx, client, hunterKey, domain)
	if err != nil {
	...
	for _, email := range hunterResp.Data.Emails {
		...
		events = append(events, NewEvent(domain, t.Name(), "email_found", props))
	...
	for _, pat := range patterns {
		...
		events = append(events, NewEvent(domain, t.Name(), "email_pattern", props))
	}

	// 4. GitHub cross-reference for developer emails
	if githubKey != "" {
		githubEvents := t.crossRefGitHub(ctx, client, githubKey, domain, hunterResp.Data.Emails, seen, mu)
...
func (t *EmailEnumTool) queryHunter(ctx context.Context, client *http.Client, apiKey, domain string) (*hunterDomainSearchResponse, error) {
	apiURL := fmt.Sprintf(
...
func (t *EmailEnumTool) crossRefGitHub(
	...
	for _, email := range devEmails {
		...
		users, err := searchGitHubUser(ctx, client, githubKey, email.Value)
		if err != nil {
		...
		for _, user := range users {
			...
			events = append(events, NewEvent(domain, t.Name(), "github_account", props))
		}
	}
...
func searchGitHubUser(ctx context.Context, client *http.Client, apiKey, email string) ([]githubUserItem, error) {
