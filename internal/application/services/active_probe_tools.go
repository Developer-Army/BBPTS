...
func (t *SSRFTool) Run(ctx context.Context, targets []string, threads int) ([]Event, error) {
	oobURL := InteractshOOBURLFromCtx(ctx)
	if oobURL == "" || len(targets) == 0 {
		return nil, nil
	}
	return processHTTPTargets(ctx, t.Name(), targets, threads, func(ctx context.Context, raw string) []Event {
		...
		for param := range q {
			...
			events = append(events, NewEventWithSeverity(testURL.String(), t.Name(), "ssrf_probe", map[string]string{
				"severity":    "info",
				"parameter":   param,
				"payload":     oobURL,
...
func (t *CRLFTool) Run(ctx context.Context, targets []string, threads int) ([]Event, error) {
	return processHTTPTargets(ctx, t.Name(), targets, threads, func(ctx context.Context, raw string) []Event {
		parsed, err := url.Parse(raw)
		if err != nil || parsed.RawQuery == "" {
			return nil
		...
		for param := range q {
			...
			if strings.EqualFold(resp.Header.Get("X-BBPTS-CRLF"), "injected") {
				return []Event{NewEventWithSeverity(testURL.String(), t.Name(), "vulnerability", map[string]string{
...
func (t *EmailHeaderInjectionTool) Run(ctx context.Context, targets []string, threads int) ([]Event, error) {
	return processHTTPTargets(ctx, t.Name(), targets, threads, func(ctx context.Context, raw string) []Event {
		parsed, err := url.Parse(raw)
		if err != nil || parsed.RawQuery == "" {
			return nil
		...
		if resp.StatusCode < 400 && (bytes.Contains(body, []byte("Bcc: injected-bbpts@example.com")) || bytes.Contains(body, []byte("injected-bbpts@example.com"))) {
			return []Event{NewEventWithSeverity(base.String(), t.Name(), "vulnerability", map[string]string{
...
func (t *APIVersionProbeTool) Run(ctx context.Context, targets []string, threads int) ([]Event, error) {
	return processHTTPTargets(ctx, t.Name(), targets, threads, func(ctx context.Context, raw string) []Event {
		parsed, err := url.Parse(raw)
		if err != nil {
			return nil
		...
		for _, version := range []string{"/api/v1/", "/api/v0/", "/api/beta/", "/api/v2/"} {
			...
			if resp.StatusCode == http.StatusOK {
				events = append(events, NewEventWithSeverity(testURL.String(), t.Name(), "discovery", map[string]string{
					"severity":    "medium",
...
func (t *JSONPTool) Run(ctx context.Context, targets []string, threads int) ([]Event, error) {
	return processHTTPTargets(ctx, t.Name(), targets, threads, func(ctx context.Context, raw string) []Event {
		parsed, err := url.Parse(raw)
		if err != nil || parsed.RawQuery == "" {
			return nil
		...
		if strings.HasPrefix(strings.TrimSpace(string(body)), "bbptsJsonp(") {
			return []Event{NewEventWithSeverity(parsed.String(), t.Name(), "vulnerability", map[string]string{
...
func (t *WebDAVTool) Run(ctx context.Context, targets []string, threads int) ([]Event, error) {
	return processHTTPTargets(ctx, t.Name(), targets, threads, func(ctx context.Context, raw string) []Event {
		...
		for _, method := range []string{"PROPFIND", "MKCOL"} {
			...
			if err == nil {
				...
				if resp.StatusCode >= 200 && resp.StatusCode < 300 {
					events = append(events, NewEventWithSeverity(base, t.Name(), "vulnerability", map[string]string{
		...
		if err == nil {
			resp.Body.Close()
			if resp.StatusCode >= 200 && resp.StatusCode < 300 {
				events = append(events, NewEventWithSeverity(putURL, t.Name(), "vulnerability", map[string]string{
...
func (t *DNSZoneTransferTool) Run(ctx context.Context, targets []string, threads int) ([]Event, error) {
	if len(targets) == 0 {
		return nil, nil
	}
	return processTextTargets(ctx, t.Name(), targets, threads, func(ctx context.Context, target string) []Event {
		...
		for _, ns := range nsRecords {
			...
			if strings.Contains(joined, "\tsoa\t") || strings.Contains(joined, " soa ") {
				events = append(events, NewEventWithSeverity(domain, t.Name(), "vulnerability", map[string]string{
...
func (t *HTTP2DowngradeTool) Run(ctx context.Context, targets []string, threads int) ([]Event, error) {
	if !ForceHTTP1FromCtx(ctx) {
		return nil, nil
	}
	return processHTTPTargets(ctx, t.Name(), targets, threads, func(ctx context.Context, raw string) []Event {
		...
		if http1Resp != http2Resp || bodyFingerprint(http1Body) != bodyFingerprint(http2Body) {
			return []Event{NewEventWithSeverity(raw, t.Name(), "discovery", map[string]string{
				"severity":     "medium",
...
func (t *TLSMisconfigTool) Run(ctx context.Context, targets []string, threads int) ([]Event, error) {
	return processTextTargets(ctx, t.Name(), targets, threads, func(ctx context.Context, target string) []Event {
		...
		for _, check := range checks {
			if strings.Contains(joined, check) && (strings.Contains(joined, "vulnerable") || strings.Contains(joined, "offered") || strings.Contains(joined, "expired") || strings.Contains(joined, "self-signed")) {
				events = append(events, NewEventWithSeverity(host, t.Name(), "vulnerability", map[string]string{
...
func processHTTPTargets(ctx context.Context, name string, targets []string, threads int, fn func(context.Context, string) []Event) ([]Event, error) {
	return processTextTargets(ctx, name, filterHTTPURLs(targets), threads, fn)
}

func processTextTargets(ctx context.Context, name string, targets []string, threads int, fn func(context.Context, string) []Event) ([]Event, error) {
	if len(targets) == 0 {
		return nil, nil
	}
...
func fetchWithTransport(ctx context.Context, raw string, useHTTP2 bool) (int, []byte) {
	tr := &http.Transport{TLSClientConfig: &tls.Config{InsecureSkipVerify: InsecureFromCtx(ctx)}}
	if useHTTP2 {
...
func nativeTLSChecks(source, host string) []Event {
	...
	for _, cert := range state.PeerCertificates {
		if time.Now().After(cert.NotAfter) {
			events = append(events, NewEventWithSeverity(host, source, "vulnerability", map[string]string{
...
var _ Tool = (*SSRFTool)(nil)
var _ Tool = (*CRLFTool)(nil)
var _ Tool = (*EmailHeaderInjectionTool)(nil)
var _ Tool = (*APIVersionProbeTool)(nil)
var _ Tool = (*JSONPTool)(nil)
var _ Tool = (*WebDAVTool)(nil)
var _ Tool = (*DNSZoneTransferTool)(nil)
var _ Tool = (*HTTP2DowngradeTool)(nil)
var _ Tool = (*TLSMisconfigTool)(nil)
