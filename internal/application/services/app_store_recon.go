...
func (t *AppStoreReconTool) Run(ctx context.Context, targets []string, threads int) ([]Event, error) {
	if len(targets) == 0 {
		return nil, nil
	}
	...
	for _, target := range targets {
		...
		go func(tgt string) {
			...
			evts, err := t.reconTarget(ctx, tgt)
			if err != nil {
...
func (t *AppStoreReconTool) reconTarget(ctx context.Context, target string) ([]Event, error) {
	events := []Event{}

	lower := strings.ToLower(target)
	switch {
	case strings.Contains(lower, "apple.com") || strings.Contains(lower, "itunes") || isNumericID(target):
		evts, err := t.searchApple(ctx, target)
		if err != nil {
			return nil, err
		}
		events = append(events, evts...)
	...
	case strings.Contains(lower, "play.google.com") || strings.Contains(lower, "market://"):
		evts := t.searchGooglePlay(ctx, target)
		events = append(events, evts...)

	default:
		ev, err := t.searchApple(ctx, target)
		if err == nil {
			events = append(events, ev...)
		}
		gp := t.searchGooglePlay(ctx, target)
...
func (t *AppStoreReconTool) searchApple(ctx context.Context, query string) ([]Event, error) {
	events := []Event{}
	...
	for _, app := range result.Results {
		...
		events = append(events, NewEvent(app.BundleID, t.Name(), "discovery", props))

		for _, u := range extractURLs(app.Description) {
			events = append(events, NewEvent(u, t.Name(), "discovery", map[string]string{
		...
		if app.SupportURL != "" {
			events = append(events, NewEvent(app.SupportURL, t.Name(), "discovery", map[string]string{
...
func (t *AppStoreReconTool) searchGooglePlay(ctx context.Context, query string) []Event {
	events := []Event{}
	...
	events = append(events, NewEvent(packageName, t.Name(), "discovery", props))

	permRe := regexp.MustCompile(`android\.permission\.([A-Z_]+)`)
	for _, p := range permRe.FindAllString(bodyStr, -1) {
		events = append(events, NewEvent(packageName, t.Name(), "discovery", map[string]string{
