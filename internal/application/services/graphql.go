...
func (g *GraphQLScanner) Run(ctx context.Context, targets []string, threads int) ([]Event, error) {
	...
	results := make(chan Event, len(targets)*len(commonGraphQLEndpoints)*4)
			...
			go func(testURL string) {
				...
				if found {
					...
					if schema != nil {
						...
						results <- NewEventWithSeverity(testURL, g.Name(), "vulnerability", map[string]string{
