// Package recon — nuclei.go provides the Nuclei vulnerability scanner adapter.
// Unlike other tools that perform discovery, Nuclei performs active vulnerability
// verification. It is placed in Stage 5 — after all discovery and crawling is complete.
package recon
...
func (t *NucleiTool) Run(ctx context.Context, targets []string, threads int) ([]Event, error) {
	...
	output, err := RunCommandWithInput(ctx, []byte(input), "nuclei", args...)
	if err != nil {
