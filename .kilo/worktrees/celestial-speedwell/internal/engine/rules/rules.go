// Package rules implements a JSON-based reactive rule engine for BBPTS.
// Rules allow the orchestrator to trigger additional tools or raise alerts
// based on specific patterns found in recon events — turning the tool from
// a passive scanner into an intelligent, reactive hunting system.
package rules
...
func (rs *RuleSet) Evaluate(events []recon.Event) ([]Match, []string) {
	var matches []Match
...
func (rs *RuleSet) matchesAll(ev recon.Event, conditions []Condition) bool {
...
func (rs *RuleSet) matchesOne(ev recon.Event, cond Condition) bool {
...
func (rs *RuleSet) resolveField(ev recon.Event, field string) string {
	switch field {
	case "target":
		return ev.Target
	case "source":
