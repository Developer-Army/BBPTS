// Package state provides scan state persistence and diffing for continuous reconnaissance.
// It stores snapshots of each scan and computes deltas between runs so you only see
// what's NEW since your last scan—the key to winning bug bounties.
package state
