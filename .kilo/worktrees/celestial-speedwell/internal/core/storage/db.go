...
func (db *DB) GetScanDiff(scope string, currentScanID int64) (*ScanDiff, error) {
	...
	rows, err := db.conn.Query(`
		SELECT host FROM targets WHERE scan_id = ? 
		AND host NOT IN (SELECT host FROM targets WHERE scan_id = ?)`,
...
func (db *DB) GetNewFindings(scope string, scanID int64) ([]EventRecord, error) {
	...
	query := `
		SELECT target, source, type, properties 
		FROM events 
		WHERE scan_id = ? 
		AND target NOT IN (SELECT target FROM events WHERE scan_id = ?)
	`
	rows, err := db.conn.Query(query, scanID, lastID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
